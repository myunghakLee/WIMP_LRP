...
class ArgoverseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_dict = {'agent_features': torch.tensor(agent_features).cuda(),
                      'ifc_helpers': ifc_helpers,
                      'social_features': torch.tensor(social_features.cuda(),
                      'social_label_features': torch.tensor(social_label_features).cuda(),
                      'adjacency': torch.tensor(adjacency).cuda(),
                      'label_adjacency': torch.tensor(label_adjacency).cuda(),
                      'num_agent_mask': torch.tensor(num_agent_mask).cuda()
                      }

        if self.mode != 'test':
            target_dict = {'agent_labels': agent_labels.cuda(),
